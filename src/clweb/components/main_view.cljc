  ...
  [:div
   ...
   [:div.ui.container {:style {:padding-right "319px" :position "relative" }}
    [:div.ui.right.internal.rail {:style {:width "319px" :padding "0px" :margin "0px"}}
     [:div.ui.segment "Logged in as " [:strong (fes/logged-in? fe-state)]
      [:div [:a {:style {:cursor "pointer"}
                 :on-click #(login-form/on-click-logout channel fe-state)}
             "Logout"]]]]
    [:div.ui.segment (fes/location fe-state)]]])
